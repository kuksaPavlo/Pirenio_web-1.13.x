...
it('statick IP test', () => {
    ...
    .should('be.visible')
    .click();
    cy.get('button[data-setting="static-ip"]')
    .should('be.visible')
    .click();
    cy.wait(7000)
    //checking DHCP conf information
    .get('[data-wan-field="ipaddr"]')
    .clear('')
    .type('192.168.1.179')
    .get('[data-wan-field="netmask"]')
    .clear('')
    .type('255.255.255.0')
    .get('[data-wan-field="gateway"]')
    .clear('')
    .type('192.168.1.1')
    .get('[data-wan-field="dns-1"]')
    .clear('')
    .type('192.168.1.1')
    .get('[data-wan-field="dns-2"]')
    .clear('')
    .type('192.168.1.1')
    chekingFooterUrls.urlsChecking()//footer url checking (AppStore, GooglePlay, Perenio.com)
    cy.get('#prev')
